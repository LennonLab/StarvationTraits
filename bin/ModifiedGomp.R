...
# Data Input
  ...
  # Creat Output
  outfile <- paste("./data/GrowthCurves/output/",output,".txt", sep="")
  ...
  # generate plot of model fits (saved)
    ...
    ci1<- round(confint(pf1)['umax',], 4)
  	} else {
      ci1 <- c(NA,NA)
		  names(ci1) <- c("2.5 %","97.5 %")
      }
    ciFI1 <- round(confint.FI(best.f1)['umax',], 4)
    ...
    write.table(results[i,], file=outfile, append=T, row.names=F,
                col.names=F, sep=",", quote=FALSE)
    }
#   results1<-results
