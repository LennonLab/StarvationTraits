...
```{r, results='hide'}
# Import Data
succ.data <- read.csv("../notes/20160209_BRBP.csv", header=T)
...
succ.data2 <- succ.data[-which(succ.data$Tube == "Voucher" | succ.data$Tube == "Control"), ]
succ.data2c <- succ.data2
succ.data2c$CPM.c <- NA

for (i in 1:dim(succ.data2c)[1]){
  if (succ.data2c$Sample[i] == "Cells"){
    succ.data2c$CPM.c[i] <- succ.data2c$CPM[i] * (6150/1500)
  } else {
  if (succ.data2c$Sample[i] == "Trap"){
    succ.data2c$CPM.c[i] <- succ.data2c$CPM[i] * (500/100)
  } else {
  if (succ.data2c$Sample[i] == "Sup"){
    succ.data2c$CPM.c[i] <- succ.data2c$CPM[i] * (6150/1500)
  }
  else {
    succ.data2c$CPM.c[i] <- NA
  }}}
}

succ.m <- melt(succ.data2c)
succ.p <- cast(succ.m, Tube + Organism + Sample ~ variable, subset=variable=="CPM.c",  fun.aggregate = c(mean, se))

succ.p$CPM_mean.p <- succ.p$CPM.c_mean / vouchers$CPM
succ.p$CPM_se.p <- succ.p$CPM.c_se / vouchers$CPM

mass.l <- melt(succ.p)

mass <- cast(mass.l, Organism ~ variable, subset=result_variable == "mean", fun.aggregate = sum)
mass$per <- mass$CPM.c / vouchers$CPM

bge.l <- subset(succ.p, succ.p$Sample == "Cells" | succ.p$Sample == "Trap")
bge.l
bge.l2 <- bge.l[,1:4]
colnames(bge.l2) <- c("Tube", "Organisms", "Sample", "CPM.c")
bge <- reshape(bge.l2, idvar = c("Tube", "Organisms"), timevar = "Sample", direction = "wide")
colnames(bge) <- c("Tube", "Organism", "Production", "Respiration")
bge$BGE <- (bge$Production) / (bge$Production + bge$Respiration)
hh

bge.m <- melt(bge.l)
bge.p <- cast(bge.m, Tube ~ Sample + value, subset=variable == "CPM.c")
for (i in levels(bge.l$Tube))
